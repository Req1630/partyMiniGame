...
void CDX9StaticMesh::Render( std::shared_ptr<CCamera> pCamera, LIGHT& Light )
{
		...
		0, &pData)))
	{
		...
		//ｶﾒﾗ位置.
		cb.vCamPos = D3DXVECTOR4( pCamera->GetPosition().x, pCamera->GetPosition().y, pCamera->GetPosition().z, 0.0f );
	...
	//ﾒｯｼｭのﾚﾝﾀﾞﾘﾝｸﾞ.
	RenderMesh( mWorld, pCamera->GetViewMatrix(), pCamera->GetProjMatrix() );
}
