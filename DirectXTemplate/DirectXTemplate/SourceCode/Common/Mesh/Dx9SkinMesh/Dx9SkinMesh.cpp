...
{
	m_mView		= pCamera->GetViewMatrix();
	m_mProj		= pCamera->GetProjMatrix();
	m_Light		= vLight;
	m_CameraPos = pCamera->GetPosition();
	m_CameraLookPos = pCamera->GetLookPosition();
...
void CDX9SkinMesh::DrawPartsMesh( SKIN_PARTS_MESH* pMesh, D3DXMATRIX World, MYMESHCONTAINER* pContainer )
{
			...
			D3D11_MAP_WRITE_DISCARD, 0, &pData ) ) )
	{
		...
		cb.vLightDir= D3DXVECTOR4( m_Light.vPos.x, m_Light.vPos.y, m_Light.vPos.z, 0 );
		cb.vEye		= D3DXVECTOR4( m_CameraPos.x, m_CameraPos.y, m_CameraPos.z, 0 );
		memcpy_s( pData.pData, pData.RowPitch, (void*)&cb, sizeof(cb) );
